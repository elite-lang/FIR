...
public:
	...
	// 用户宏的查找与声明设置
	virtual MacroModel* getUserMacro(const std::string& name) = 0;
	virtual void setUserMacro(const std::string& name, Node* node) = 0;

	virtual FunctionModel* getFunctionModel(const std::string& name) = 0;
	virtual StructModel* getStructModel(const std::string& name) = 0;
