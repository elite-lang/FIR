...
public:
	...
	// 用户宏的查找与声明设置
	virtual MacroModel* getUserMacro(const std::string& name);
	virtual void setUserMacro(const std::string& name, Node* node);

	virtual FunctionModel* getFunctionModel(const std::string& name);
	virtual StructModel* getStructModel(const std::string& name);
