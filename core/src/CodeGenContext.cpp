...
LValue CodeGenContext::MacroMake(Node* node) {
	...
	if (node->isIDNode()) {
		MacroModel* p = getUserMacro(node->getStr());
		if (p != NULL) {
...
FunctionModel* CodeGenContext::getFunctionModel(const string& name) {
	id* i = FindST(name);
	if (i == NULL) {
		return NULL;
	}
	...
	return dynamic_cast<FunctionModel*>(i->data);
}

MacroModel* CodeGenContext::getUserMacro(const std::string& name) {
	id* i = FindST(name);
	if (i == NULL) return NULL;
	if (i->type != macro_t) return NULL;
	return dynamic_cast<MacroModel*>(i->data);
}
...
StructModel* CodeGenContext::getStructModel(const string& name) {
	id* i = FindST(name);
	if (i == NULL) {
		printf("错误: 符号 %s 未找到\n", name.c_str());
		return NULL;
	...
	return dynamic_cast<StructModel*>(i->data);
}
...
LValue CodeGenContext::FindSrcType(const string& name) {
	...
	if (i != NULL && i->type == type_t) {
		...
		auto s = dynamic_cast<StructModel*>(i->data);
